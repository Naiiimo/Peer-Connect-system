...
export const Route = createFileRoute("/_authenticated/tutor/notifications")({ component: Notif });
function Notif() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    load();
    const ch = supabase.channel("tnotif").on("postgres_changes", { event: "*", schema: "public", table: "notifications", filter: `user_id=eq.${user?.id}` }, load).subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [user]);
  const markAll = async () => { if (!user) return; await supabase.from("notifications").update({ read_at: new Date().toISOString() }).eq("user_id", user.id).is("read_at", null); load(); };
  const open = async (n: any) => {
    if (!n.read_at) await supabase.from("notifications").update({ read_at: new Date().toISOString() }).eq("id", n.id);
    const to = n.link || (n.kind === "message" ? "/tutor/messages" : "/tutor");
    navigate({ to });
  };
  return (
    <div>
