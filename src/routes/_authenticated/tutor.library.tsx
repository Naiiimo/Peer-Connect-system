...
function Lib() {
  ...
  return (
    <div>
      <PageHeader title="Teaching library" description="Materials you share with students." actions={
        <label><input type="file" hidden onChange={(e) => e.target.files?.[0] && upload(e.target.files[0])} /><Button asChild><span><Upload className="mr-1 h-4 w-4" /> Upload</span></Button></label>
      } />
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        {docs.map((d) => (
          <div key={d.id} className="card-elevated flex items-center gap-3 p-4">
            ...
            <div className="min-w-0 flex-1"><div className="truncate text-sm font-medium">{d.name}</div></div>
